...
from gem_navigation.msg import StringStamped
...
class PurePursuit(object):
    ...
    def __init__(self):
        ...
        self.path_points_x = None
        self.response_calculated = 0
        ...
        self.waypoints_sub = rospy.Subscriber('/planner_waypoints', PoseArray, self.waypoints_callback)
        self.car_status_sub = rospy.Subscriber('/car_status', StringStamped, self.car_status_callback)
    ...
    def car_status_callback(self, msg):
        ...
        self.message_time = msg.header.stamp

    def waypoints_callback(self, msg):
        self.path_points_x = [pose.position.x for pose in msg.poses]
        self.path_points_y = [pose.position.y for pose in msg.poses]
        self.path_points_yaw = [pose.position.z for pose in msg.poses]
    ...
    def start_pp(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.car_status == "ERROR" or self.car_status == "IDLE":
                self.ackermann_msg.speed = 0
                if not self.response_calculated:
                    response_time = rospy.Time.now()
                    print("Response Time - ", (response_time.to_sec() - self.message_time.to_sec()))
                    self.response_calculated = 1
            elif self.car_status == "RUNNING":
                self.ackermann_msg.speed = 2.8
                if self.response_calculated:
                    response_time = rospy.Time.now()
                    print("Recovery Time - ", (response_time.to_sec() - self.message_time.to_sec()))
                self.response_calculated = 0
